...
const styles = {
  ...
  gradients: {
    ...
    darkOverlay: 'linear-gradient(135deg, rgba(2,8,24,0.9) 0%, rgba(6,15,46,0.85) 100%)',
  },
  shadows: {
...
const Propre = () => {
  return (
    <>
      ...
      {/* ==================== HERO ==================== */}
      <section className="relative min-h-screen flex items-center overflow-hidden grid-bg" style={{ background: styles.colors.navy[950] }}>
        {/* Orbs */}
        ...
        <div className="absolute inset-0 z-0">
          <div className="absolute inset-0 bg-cover bg-center opacity-40" style={{ backgroundImage: `url('https://img.freepik.com/photos-gratuite/contexte-energie-nucleaire-ia-innovation-future-technologie-rupture_53876-129783.jpg?semt=ais_hybrid&w=2000&q=90')` }} />
          <div className="absolute inset-0" style={{ background: styles.gradients.darkOverlay }} />
        </div>
            ...
            <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
              <motion.div variants={scaleIn}>
                <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full" style={{ background: 'rgba(37,99,235,0.15)', border: '1px solid rgba(37,99,235,0.3)' }}>
                  ...
                  <span className="tag text-blue-300" style={{ fontFamily: "'Syne', sans-serif", fontSize: '0.7rem', fontWeight: 600, letterSpacing: '0.15em' }}>Solutions Digitales Premium – Afrique</span>
                </div>
              </motion.div>
              <motion.h1 variants={fadeUp} className="font-display font-extrabold text-6xl sm:text-7xl text-white leading-tight mb-6" style={{ fontFamily: "'Syne', sans-serif", lineHeight: 1.05 }}>
                Transformez votre<br />
                <span className="text-gradient" style={{ background: 'linear-gradient(135deg, #60a5fa, #2563eb, #818cf8)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>Business Digital</span><br />
                avec l'expertise
              </motion.h1>
              <motion.p variants={fadeUp} className="text-slate-400 text-xl leading-relaxed max-w-2xl mb-10">
                ERP • SaaS • Cybersécurité • Développement Web & Mobile.<br />
                Omedev vous accompagne dans votre <strong className="text-white">digitalisation complète</strong>, de la stratégie au déploiement.
              </motion.p>
              <motion.div variants={fadeUp} className="flex flex-wrap gap-4 mb-16">
                <Link to="/contact" className="btn-primary"><span className="relative z-10 flex items-center gap-2">Demander un devis <ArrowRight size={18} /></span></Link>
                <Link to="/solutions" className="btn-outline flex items-center gap-2"><span className="relative z-10">Voir les solutions <ArrowRight size={16} /></span></Link>
                <Link to="/audit" className="flex items-center gap-2 text-green-400 font-display font-semibold px-6 py-4 rounded-xl text-base border border-green-500/30 hover:bg-green-500/10 transition-colors">🎁 Audit Gratuit</Link>
              </motion.div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[{ n: '150+', l: 'Projets livrés' }, { n: '98%', l: 'Satisfaction client' }, { n: '50+', l: 'Entreprises clientes' }, { n: '5 ans', l: "D'expertise" }].map((s, i) => (
                  <div key={i} className="stat-card rounded-xl p-4 text-center" style={{ background: 'linear-gradient(135deg, rgba(37,99,235,0.1), rgba(37,99,235,0.03))', border: '1px solid rgba(37,99,235,0.2)' }}>
                    <div className="font-display font-extrabold text-2xl text-white mb-1">{s.n}</div>
                    <div className="text-slate-500 text-xs font-body">{s.l}</div>
                  </div>
                ))}
              </div>
            </motion.div>
      ...
      {/* ==================== STATS ==================== */}
      <section className="py-20" style={{ background: styles.gradients.primary }}>
        <div className="container">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {stats.map((stat, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }}
                className="p-6 rounded-2xl backdrop-blur-sm" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
      ...
      {/* ==================== CTA FINALE ==================== */}
      <section className="py-24 relative overflow-hidden" style={{ background: styles.gradients.primary }}>
        <div className="container text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            ...
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">Prêt à transformer votre entreprise ?</h2>
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              ...
              <Link to="/audit-gratuit" className="btn-outline text-white border-white/30 hover:bg-white/10">Audit gratuit <CheckCircle size={18} /></Link>
              <Link to="/boutique" className="btn-outline text-white border-white/30 hover:bg-white/10">Voir nos produits <ChevronRight size={18} /></Link>
            </div>
          </motion.div>
        </div>
      </section>
