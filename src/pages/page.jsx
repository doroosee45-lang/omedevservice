...
const Home = () => {
  ...
  // Produits & Équipements (Nouvelle section)
  const products = [
    {
      icon: ThermometerSun,
      title: 'Climatisation Professionnelle',
      description: 'Climatiseurs split, VRV, gainables et systèmes haute performance pour bureaux et entreprises',
      color: 'from-sky-500 to-cyan-500'
    },
    {
      icon: Monitor,
      title: 'Ordinateurs & Matériel IT',
      description: 'PC de bureau, laptops professionnels, serveurs, écrans et accessoires premium',
  ...
  const packs = [
    { name: 'Pack Entreprise', price: 'Sur devis', features: ['Réseau complet + Sécurité', 'ERP sur mesure', 'Support prioritaire', 'Maintenance incluse'], icon: Briefcase },
    { name: 'Pack E-commerce', price: 'Sur devis', features: ['Site performant + Mobile', 'Paiement intégré', 'SEO local', 'Hébergement cloud'], icon: Globe },
    { name: 'Pack Digital Complet', price: 'Sur devis', features: ['Site + Application', 'Télécom & Surveillance', 'Cloud sécurisé', 'Formation équipe'], icon: Cpu },
  ];
  ...
  return (
    <div className="overflow-x-hidden">
        ...
        <div className="container-custom relative z-10">
          <div className="max-w-5xl mx-auto text-center">
            <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
              <motion.div variants={fadeInUp} className="inline-flex items-center gap-3 bg-white/10 backdrop-blur-2xl border border-white/20 px-7 py-3 rounded-3xl mb-8">
                <div className="w-2.5 h-2.5 bg-emerald-400 rounded-full animate-pulse" />
                <span className="uppercase tracking-[3px] text-sm font-medium">OMDEVE Services • Kinshasa</span>
              </motion.div>

              <motion.h1 variants={fadeInUp} className="text-6xl md:text-7xl lg:text-8xl font-black leading-none tracking-tighter mb-8">
                Votre partenaire technologique<br />
                <span className="bg-gradient-to-r from-cyan-300 via-sky-300 to-blue-300 bg-clip-text text-transparent">de confiance en Afrique</span>
              </motion.h1>

              <motion.p variants={fadeInUp} className="text-2xl text-white/80 max-w-3xl mx-auto mb-14 leading-relaxed">
                Développement • Télécom • Vidéosurveillance • Climatisation • Matériel IT & Maintenance
              </motion.p>

              <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-5 justify-center">
                <Link to="/demander-devis" className="btn-primary text-lg px-12 py-5 rounded-3xl flex items-center gap-3 shadow-2xl hover:shadow-white/20">
                  Demander un devis
                  <ArrowRight className="w-6 h-6" />
                </Link>
                <Link to="/audit-gratuit" className="group border-2 border-white/80 hover:border-white px-12 py-5 rounded-3xl font-semibold text-lg flex items-center gap-3 transition-all hover:bg-white hover:text-zinc-900">
                  Audit gratuit
      ...
      <section className="py-24 bg-gray-50">
        <div className="container-custom">
          <div className="text-center mb-16">
            <div className="inline-flex bg-amber-100 text-amber-700 px-6 py-3 rounded-3xl text-sm font-semibold mb-4">PRODUITS & ÉQUIPEMENTS</div>
            <h2 className="section-title">Achetez vos équipements chez nous</h2>
            <p className="section-subtitle">Climatiseurs professionnels • PC & Matériel IT • Caméras de surveillance haute qualité</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {products.map((product, idx) => (
              ...
              >
                ...
                <div className="p-10">
                  <div className={`w-20 h-20 bg-gradient-to-br ${product.color} rounded-3xl flex items-center justify-center mb-8 shadow-xl group-hover:scale-110 transition-transform duration-400`}>
                    <product.icon className="w-11 h-11 text-white" />
                  </div>
                  <h3 className="text-3xl font-bold mb-4">{product.title}</h3>
                  <p className="text-gray-600 leading-relaxed mb-8">{product.description}</p>
                  <Link to="/boutique" className="inline-flex items-center font-semibold text-primary-600 hover:text-primary-700 group-hover:gap-3 transition-all">
      ...
      <section className="py-24 bg-white">
        <div className="container-custom">
          <div className="text-center mb-16">
            <div className="inline-flex bg-primary-100 text-primary-700 px-6 py-3 rounded-3xl text-sm font-semibold mb-4">EXPERTISE TECHNIQUE</div>
            <h2 className="section-title">Domaines d'excellence</h2>
            <p className="section-subtitle">Nous maîtrisons l’ensemble de votre écosystème technologique</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {expertise.map((item, idx) => (
              ...
              >
                <div className={`w-20 h-20 bg-gradient-to-br ${item.color} rounded-3xl flex items-center justify-center mb-8 shadow-lg group-hover:scale-110 transition-transform`}>
      ...
      <section className="py-24 bg-gray-50 relative">
        ...
        <div className="container-custom">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 bg-primary-100 text-primary-700 px-6 py-3 rounded-3xl text-sm font-semibold mb-4">NOS SERVICES</div>
            <h2 className="section-title">Services Intégrés</h2>
            <p className="section-subtitle">Une expertise complète pour votre transformation technologique</p>
          </div>
          ...
          <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, idx) => (
              ...
              >
                <div className="p-10">
                  <div className={`w-20 h-20 bg-gradient-to-br ${service.color} rounded-3xl flex items-center justify-center mb-8 shadow-xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-400`}>
                    <service.icon className="w-11 h-11 text-white" />
                  </div>
                  <h3 className="text-2xl font-bold mb-4">{service.title}</h3>
                  <p className="text-gray-600 mb-8 leading-relaxed">{service.description}</p>
                  <Link to={service.link} className="inline-flex items-center font-semibold text-primary-600 hover:text-primary-700 group-hover:gap-3 transition-all">
                    Découvrir <ArrowRight className="ml-2 w-5 h-5 transition-transform group-hover:translate-x-1" />
                  </Link>
                </div>
                <div className={`h-1.5 bg-gradient-to-r ${service.color} scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-500`} />
              </motion.div>
            ))}
          </motion.div>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {packs.map((pack, idx) => (
              ...
              >
                ...
                <Link to="/demander-devis" className="btn-primary w-full py-4 text-center block rounded-2xl group-hover:scale-[1.02] transition-transform">
                  Demander ce pack
                </Link>
              </motion.div>
            ))}
      ...
      <section className="py-24 bg-white">
        <div className="container-custom">
          <div className="text-center mb-16">
            ...
            <p className="section-subtitle">Engagement, expertise locale et résultats mesurables</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: CheckCircle, title: "Expertise locale & internationale", desc: "Connaissance approfondie du contexte africain et standards mondiaux" },
              { icon: Clock, title: "Support 24/7 réactif", desc: "Équipe dédiée disponible en permanence pour vos besoins critiques" },
              { icon: Award, title: "Qualité & Garantie", desc: "Solutions durables avec SLA et maintenance proactive" },
            ].map((item, idx) => (
      ...
      <section className="py-28 bg-gradient-to-br from-primary-700 via-primary-600 to-secondary-600 text-white relative overflow-hidden">
        <div className="container-custom text-center relative z-10">
          <h2 className="text-5xl md:text-6xl font-bold mb-6">Prêt à équiper et transformer votre entreprise ?</h2>
          <p className="text-2xl text-white/90 mb-12 max-w-2xl mx-auto">Climatisation • Matériel IT • Caméras • Solutions digitales complètes</p>

          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <Link to="/demander-devis" className="btn-primary text-xl px-14 py-6 rounded-3xl">Demander un devis</Link>
            <Link to="/audit-gratuit" className="border-2 border-white/90 hover:bg-white hover:text-primary-700 px-14 py-6 rounded-3xl text-xl transition-all">Audit gratuit</Link>
            <Link to="/boutique" className="border-2 border-white/90 hover:bg-white hover:text-primary-700 px-14 py-6 rounded-3xl text-xl transition-all">Voir nos produits</Link>
          </div>
        </div>
      </section>
    </div>
