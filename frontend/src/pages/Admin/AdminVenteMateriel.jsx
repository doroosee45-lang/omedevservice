...
import {
  ShoppingBag, Package, Clock, X, Plus, Edit2, Trash2, Eye,
  RefreshCw, DollarSign, Save, ToggleLeft, ToggleRight,
...
import { PageHeader, Button, Tabs, SearchInput, EmptyState, LoadingState } from '../../components/Admin/ui'
...
const ORDER_STATUSES = [
  ...
  { value: 'delivered',  label: 'Livrée',        color: 'bg-[#2AACB2]/20 text-[#55DDB5] border-[#2AACB2]/30' },
...
const PAYMENT_STATUSES = [
  ...
  { value: 'paid',     label: 'Payé',       color: 'text-[#55DDB5]' },
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
      className={`fixed bottom-6 right-6 z-[9999] flex items-center gap-3 px-5 py-3 rounded-2xl shadow-2xl border text-sm font-medium
        ${type === 'success' ? 'bg-[#053876]/90 border-[#2AACB2]/40 text-[#55DDB5]' : 'bg-red-900/90 border-red-500/40 text-red-300'}`}>
  ...
  return (
    <div className="fixed inset-0 admin-modal-overlay z-50 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        className="admin-modal-panel w-full max-w-xl max-h-[92vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-5 border-b border-white/10 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {isEdit && <p className="text-xs text-white/40 font-mono">{product.reference}</p>}
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-white/50 hover:text-white transition">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-white/30 mt-1">Mettre 0 pour afficher "Rupture de stock"</p>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-white/30 mt-1">Une spécification par ligne · Format : <span className="font-mono text-white/40">Clé: Valeur</span></p>
          </div>
          ...
          <div className="flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-xl">
            <div>
              ...
              <p className="text-xs text-white/40 mt-0.5">Désactiver cache le produit du catalogue public</p>
            </div>
            <button onClick={() => set('isActive', !form.isActive)} className="transition-transform hover:scale-110">
              {form.isActive
                ? <ToggleRight className="w-8 h-8 text-[#55DDB5]" />
        ...
        <div className="flex gap-3 p-5 border-t border-white/10 flex-shrink-0">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-white/20 text-white/70 hover:bg-white/10 transition text-sm">
            Annuler
          </button>
          <button onClick={handleSave} disabled={saving}
            className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-[#0B74C1] to-[#2AACB2] text-white font-semibold text-sm hover:from-[#1D5B9B] hover:to-[#55DDB5] transition disabled:opacity-50 flex items-center justify-center gap-2">
...
const OrderModal = ({ order, onClose, onSaved }) => {
  ...
  return (
    <div className="fixed inset-0 admin-modal-overlay z-50 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        className="admin-modal-panel w-full max-w-2xl max-h-[92vh] flex flex-col">

        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-white/10 flex-shrink-0">
          <div>
            <h2 className="text-base font-bold text-white font-syne font-mono">{order.orderNumber}</h2>
            <p className="text-white/50 text-xs mt-0.5">{new Date(order.createdAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <p className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-3">Produit commandé</p>
              ...
              <div className="mt-4 space-y-1.5 text-sm">
                <div className="flex justify-between"><span className="text-white/50">Réf. produit</span><span className="font-mono text-white/70 text-xs">{order.productSnapshot?.reference}</span></div>
                <div className="flex justify-between"><span className="text-white/50">Prix unitaire</span><span className="text-white">{order.unitPrice?.toLocaleString('fr-FR')} €</span></div>
                <div className="flex justify-between"><span className="text-white/50">Quantité</span><span className="text-white">× {order.quantity}</span></div>
                <div className="flex justify-between border-t border-white/10 pt-2 mt-2">
                  <span className="font-bold text-white">Total</span>
                  <span className="text-[#55DDB5] font-bold text-base">{order.totalPrice?.toLocaleString('fr-FR')} €</span>
                </div>
              </div>
            </div>

            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <p className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-3">Informations client</p>
              <div className="space-y-2.5 text-sm">
                <div className="flex items-center gap-2.5 text-white/80"><User className="w-4 h-4 text-blue-400 flex-shrink-0" />{order.fullName}</div>
                <div className="flex items-center gap-2.5 text-white/80"><Mail className="w-4 h-4 text-blue-400 flex-shrink-0" />{order.email}</div>
                <div className="flex items-center gap-2.5 text-white/80"><Phone className="w-4 h-4 text-blue-400 flex-shrink-0" />{order.phone}</div>
                <div className="flex items-start gap-2.5 text-white/80"><MapPin className="w-4 h-4 text-blue-400 flex-shrink-0 mt-0.5" />
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-4">
            <p className="text-xs font-semibold text-white/40 uppercase tracking-wider">Gestion de la commande</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-white/50 mb-1.5">Statut commande</label>
              ...
              <div>
                <label className="block text-xs text-white/50 mb-1.5">Statut paiement</label>
            ...
            <div>
              <label className="block text-xs text-white/50 mb-1.5">Notes internes (visibles seulement par l'admin)</label>
        ...
        <div className="flex gap-3 p-5 border-t border-white/10 flex-shrink-0">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-white/20 text-white/70 hover:bg-white/10 transition text-sm">Fermer</button>
          <button onClick={handleSave} disabled={saving}
            className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-[#0B74C1] to-[#2AACB2] text-white font-semibold text-sm hover:from-[#1D5B9B] hover:to-[#55DDB5] transition disabled:opacity-50 flex items-center justify-center gap-2">
            {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Catalogue produits & gestion des commandes clients"
        actions={
          <>
            <Button variant="outline" icon={RefreshCw} onClick={loadData}>Actualiser</Button>
            {activeTab === 'products' && (
              <Button variant="primary" icon={Plus} onClick={() => setProductModal(null)}>Ajouter un produit</Button>
            )}
          </>
        }
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard icon={DollarSign}  label="Chiffre d'affaires"    value={stats ? `${(stats.revenue || 0).toLocaleString('fr-FR')} €` : '—'} sub={`${stats?.delivered ?? 0} livrées`} color="bg-[#2AACB2]/20 text-[#55DDB5]" />
      </div>

      {/* Tabs */}
      <Tabs
        tabs={[
          { key: 'products', label: 'Catalogue produits', icon: Package,     count: products.length },
          { key: 'orders',   label: 'Commandes',          icon: ShoppingBag, count: orders.length   },
        ]}
        active={activeTab}
      ...
      {activeTab === 'products' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <SearchInput value={prodSearch} onChange={e => setProdSearch(e.target.value)} placeholder="Rechercher par nom, référence…" />
            <div className="flex gap-2">
              <button onClick={() => setViewMode('grid')} title="Vue cartes"
                className={`p-2.5 rounded-xl border text-sm transition ${viewMode === 'grid' ? 'bg-blue-500/20 border-blue-500/40 text-blue-400' : 'border-white/10 text-white/40 hover:text-white/80'}`}>
                <LayoutGrid className="w-4 h-4" />
              </button>
              <button onClick={() => setViewMode('table')} title="Vue liste"
                className={`p-2.5 rounded-xl border text-sm transition ${viewMode === 'table' ? 'bg-blue-500/20 border-blue-500/40 text-blue-400' : 'border-white/10 text-white/40 hover:text-white/80'}`}>
          ...
          <div className="flex flex-wrap gap-2">
            {['Tous', ...CATEGORIES].map(cat => (
              <button key={cat} onClick={() => setProdCategory(cat)}
                className={`px-3.5 py-1.5 rounded-xl text-xs font-medium transition ${prodCategory === cat ? 'bg-blue-500 text-white' : 'bg-white/5 text-white/50 hover:bg-white/10 border border-white/10'}`}>
          ...
          {loading ? (
            ...
            <EmptyState
              ...
              action={<button onClick={() => setProductModal(null)} className="text-blue-300 text-sm hover:underline">+ Ajouter le premier produit</button>}
            />
          ) : viewMode === 'grid' ? (
            /* Vue cartes */
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                <motion.div key={product._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="relative h-40 bg-slate-800 overflow-hidden">
                    ...
                    <div className="absolute top-2 right-2">
                      <button onClick={() => handleToggleProduct(product)} title="Activer / désactiver"
                        className={`text-xs font-bold px-2 py-0.5 rounded-full border transition ${product.isActive ? 'bg-[#2AACB2]/80 border-[#2AACB2] text-white' : 'bg-gray-600/80 border-gray-500 text-white/70'}`}>
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      <span className="text-[#55DDB5] font-bold text-sm flex-shrink-0">{product.price?.toLocaleString('fr-FR')} €</span>
                    </div>
                    <p className="text-xs text-white/40 font-mono mb-2">{product.reference}</p>
                    <p className="text-white/50 text-xs leading-relaxed mb-3 line-clamp-2">{product.description}</p>
                    <div className="flex items-center justify-between">
                      <span className={`text-xs font-medium ${product.stock <= 2 ? 'text-red-400' : product.stock <= 5 ? 'text-amber-400' : 'text-white/50'}`}>
              ...
              <motion.button onClick={() => setProductModal(null)} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className="h-full min-h-60 flex flex-col items-center justify-center gap-3 bg-white/3 border-2 border-dashed border-white/15 rounded-2xl hover:border-blue-500/40 hover:bg-blue-500/5 transition-all text-white/40 hover:text-blue-400 group">
              ...
              <div className="overflow-x-auto">
                <table className="admin-table">
                  ...
                  <tbody >
                    {filteredProducts.map(product => (
                      <tr key={product._id}>
                        <td className="font-mono text-xs text-white/40">{product.reference}</td>
                        <td>
                          <div className="flex items-center gap-3">
                            {product.image
                              ? <img src={product.image} alt="" className="w-9 h-9 rounded-lg object-cover flex-shrink-0" />
                              : <div className="w-9 h-9 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0"><ImageIcon className="w-4 h-4 text-white/40" /></div>
                            }
                            <div>
                              <p className="text-white text-sm font-medium">{product.name}</p>
                              <p className="text-white/40 text-xs truncate max-w-48">{product.description}</p>
                            </div>
                          </div>
                        </td>
                        <td>
                        ...
                        <td className="text-[#55DDB5] font-semibold text-sm">{product.price?.toLocaleString('fr-FR')} €</td>
                        <td>
                          <span className={`text-sm font-medium ${product.stock <= 2 ? 'text-red-400' : product.stock <= 5 ? 'text-amber-400' : 'text-white/70'}`}>
                            {product.stock}
                          </span>
                        </td>
                        <td>
                          <button onClick={() => handleToggleProduct(product)}>
                            {product.isActive
                              ? <span className="text-xs bg-[#2AACB2]/20 text-[#55DDB5] border border-[#2AACB2]/30 px-2.5 py-1 rounded-full font-medium">Actif</span>
                              : <span className="text-xs bg-gray-500/20 text-white/50 border border-gray-500/30 px-2.5 py-1 rounded-full font-medium">Inactif</span>
                            }
                          </button>
                        </td>
                        <td>
      ...
      {activeTab === 'orders' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          {/* Filtres */}
          <div className="flex flex-col sm:flex-row gap-3">
            <SearchInput value={orderSearch} onChange={e => setOrderSearch(e.target.value)} placeholder="N° commande, nom client, email, produit…" />
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
              className="admin-select text-sm min-w-40">
                ...
                <table className="admin-table">
                  ...
                  <tbody >
                    {filteredOrders.length === 0 ? (
                      <tr><td colSpan={9} className="text-center text-white/40 py-14">Aucune commande trouvée.</td></tr>
                    ) : filteredOrders.map(order => {
                      const pc = PAYMENT_STATUSES.find(s => s.value === order.paymentStatus)
                      return (
                        <tr key={order._id} className="cursor-pointer" onClick={() => setOrderModal(order)}>
                          <td><span className="font-mono text-blue-400 text-sm font-medium">{order.orderNumber}</span></td>
                          <td>
                            <p className="text-white text-sm font-medium">{order.fullName}</p>
                            <p className="text-white/40 text-xs">{order.email}</p>
                          </td>
                          <td>
                            <p className="text-white/70 text-sm">{order.productSnapshot?.name || '—'}</p>
                            <p className="text-white/40 text-xs">{order.productSnapshot?.category}</p>
                          </td>
                          <td className="text-white/70 text-sm">×{order.quantity}</td>
                          <td><span className="text-[#55DDB5] font-semibold text-sm">{order.totalPrice?.toLocaleString('fr-FR')} €</span></td>
                          <td><span className={`text-xs font-medium ${pc?.color || 'text-white/50'}`}>{pc?.label || '—'}</span></td>
                          <td><StatusBadge status={order.status} /></td>
                          <td className="text-white/50 text-xs whitespace-nowrap">{new Date(order.createdAt).toLocaleDateString('fr-FR')}</td>
